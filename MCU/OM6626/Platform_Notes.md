...
OM6626 平台 SDK 在连接断开后**不会自动恢复广播**。这与另一平台的行为存在差异（另一平台可通过 `disconnectNoRestart` 控制）。
...
- OM6626 平台 SDK 没有公开 Resolve List（RL）管理 API，Controller 无法自动把 RPA 解析成 identity 地址再匹配白名单。
...
但 OM6626 平台 SDK 广播状态机本身不稳定，反复重启广播容易失败。
...
- [儿童座椅 BLE 白名单调试](../../Projects/儿童座椅/WhiteList_Debugging_Journey.md)
